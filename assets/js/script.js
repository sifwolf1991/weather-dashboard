...
// FETCH REQUEST 1 to Open Weather Map 
...
console.log("there is a json object: currentData");
...
// integration of second API call:
var latitude = currentData.coord.lat
var longitude = currentData.coord.lon;

var geoURL = `https://api.openweathermap.org/geo/1.0/reverse?lat=${latitude}&longitude=${longitude}&limit=5&appid=${API_KEY}`;
fetch(geoURL)
.then (function (response2) {
console.log ("Geo call has a response");
return response2.json();
})
.then (function (data2) {
console.log(data2) ("geo json has a return");
})
...
// fetch request for the next 4 days forecasts
            fetch(`https://api.openweathermap.org/data/2.5/forecast?q=${searchTerm}&appid=${API_KEY}&units=metric&lat=-37.814&lon=144.9633`)
            .then(function (response) {
            return response.json();
            })
            .then(function (data) {
